export function renderBaseStationPop(app, data = {}, navigate) {
  const {
    orgName = "WinterWord",
  ...
  const popAvailable = popClueLive === true;
...
.ww-pop-tooltip{ top:29.9%; }
.ww-clue-tooltip{ top:44%; }
.ww-life-tooltip{ top:57.6%; }
.ww-leader-tooltip{ top:71.1%; }

.ww-pop-hotspot:hover ~ .ww-pop-tooltip,
.ww-pop-hotspot:focus-visible ~ .ww-pop-tooltip,
...
<div id="wwPortal">
  ...
  <div id="wwStage" aria-label="WinterWord Base Station">
    ...
    <img
      class="ww-shell"
      src="/assets/winterword/shared/BS2.png"
    ...
    <button
      class="ww-hotspot ww-pop-hotspot"
      type="button"
      data-nav="pop"
      data-disabled="${popAvailable ? "false" : "true"}"
      aria-label="${popAvailable ? "Open POP clue" : "POP clue unavailable"}"
    ></button>
    <img
      class="ww-icon ww-pop-icon"
      src="/assets/winterword/shared/icon_pop.png"
      alt=""
      aria-hidden="true"
      draggable="false"
    >
    ...
    <div class="ww-tooltip ww-pop-tooltip ${popAvailable ? "" : "ww-tooltip--locked"}">
      <span class="ww-tooltip-title">POP</span>
      ${
        popAvailable
          ? `A temporary signal has broken through the snow. Follow it before it vanishes.`
          : `No POP clue is live right now.`
      }
    </div>
